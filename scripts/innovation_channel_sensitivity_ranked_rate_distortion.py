#!/usr/bin/env python
"""Task (7) variant: keep whole latent channels by channel sensitivity rank.

This mirrors scripts/eval_rate_distortion.py, but replaces the prefix-Kp mask
with a channel-sensitivity mask. One latent channel contains 8*8 = 64 Kp
elements, so Kp=128 keeps the top 2 sensitive channels, Kp=256 keeps the top 4,
..., and Kp=1024 keeps all 16 channels.

Example:
  python scripts/innovation_channel_sensitivity_ranked_rate_distortion.py \
      --data_path /home/lc/class/yuyi/cifar-10 \
      --ckpt outputs/jscc/snr_7/best_jscc_snr7.pt \
      --channel_sensitivity_rank_indices outputs/innovation/channel_sensitivity/channel_sensitivity_rank_indices.npy \
      --out_dir outputs/innovation/channel_sensitivity_ranked_task7 \
      --num_images 500 \
      --batch_size 256
"""
...
from eval_rate_distortion import INPUT_K, LATENT_C, LATENT_H, LATENT_W, load_jscc_checkpoint, parse_kp_list
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Evaluate task7 rate-distortion with channel-sensitivity-ranked whole-channel retention."
    )
    ...
    parser.add_argument(
        "--channel_sensitivity_rank_indices",
        default="outputs/innovation/channel_sensitivity/channel_sensitivity_rank_indices.npy",
        help="Path to channel_sensitivity_rank_indices.npy produced by innovation_channel_sensitivity.py.",
    )
    parser.add_argument("--out_dir", default="outputs/innovation/channel_sensitivity_ranked_task7")
...
def load_channel_sensitivity_rank_indices(path: Path) -> np.ndarray:
    if not path.is_file():
        raise FileNotFoundError(
            "Channel sensitivity rank file not found. Run scripts/innovation_channel_sensitivity.py first, "
            f"or pass --channel_sensitivity_rank_indices explicitly. Missing: {path}"
        )
...
@torch.no_grad()
def evaluate_kp_channel_sensitivity_ranked(
    model: DeepJSCC,
    ...
    """Evaluate one Kp using normalized latent, sensitivity top-M channel mask, AWGN, and decoder."""
...
def write_summary(
    ...
    lines = [
        "Task (7) channel-sensitivity-ranked rate-distortion summary",
        "=" * 68,
        f"Checkpoint: {ckpt_path}",
        f"Channel sensitivity rank indices: {rank_path}",
        f"Sensitivity-ranked channel order: {rank_indices.tolist()}",
        f"Shared sampled test images: {num_images}",
        f"Test SNR: {test_snr_db:g} dB",
        f"Mask strategy: keep whole channels by channel sensitivity rank; 1 channel = {ELEMENTS_PER_CHANNEL} Kp elements",
    ...
    for row in rows:
        ...
        kept = rank_indices[:channels].tolist()
        lines.append(
...
def main() -> None:
    ...
    rank_path = resolve_path(args.channel_sensitivity_rank_indices, project_dir)
    ...
    rank_np = load_channel_sensitivity_rank_indices(rank_path)
    rank_tensor = torch.from_numpy(rank_np).long()
    ...
    for kp in kp_values:
        ...
        metrics = evaluate_kp_channel_sensitivity_ranked(model, dataloader, device, kp, args.test_snr_db, rank_tensor)
        rows.append(metrics)
        channels = kp // ELEMENTS_PER_CHANNEL
        print(
    ...
    write_summary(summary_path, rows, len(samples.images), args.test_snr_db, ckpt_path, rank_path, rank_np)
